...
class InvoiceFeesModel extends \Model\BaseModel
{
    ...
    public function getData($data = array())
    {
        $sql = 'SELECT t.*, i.serie AS invoice_serie, i.nr AS invoice_nr, i.status AS invoice_status, i.config AS invoice_configs, i.paid_at, i.delivered_at,
            w.title AS warehouse_name, (SELECT SUM(ii.price*ii.quantity) FROM {tablePrefix}ext_invoice_item ii WHERE ii.invoice_id = t.invoice_id) AS total_revenue, SUM(t.fee) AS total_fee, COUNT(t.id) AS total_transaction     
